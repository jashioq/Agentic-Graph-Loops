...
def test_transition_moves_the_ticket() -> None:
    subject = ticket()

    transition(subject, Status.IN_PROGRESS)

    assert subject.status is Status.IN_PROGRESS


def test_an_illegal_transition_raises_and_leaves_the_ticket_alone() -> None:
    subject = ticket()
...
def test_awaiting_input_remembers_where_it_came_from_and_returns_there(frm: Status) -> None:
    subject = ticket(status=frm)

    transition(subject, Status.AWAITING_INPUT)
    assert subject.status is Status.AWAITING_INPUT
    assert subject.resume_to is frm

    transition(subject, frm)
    assert subject.status is frm
    assert subject.resume_to is None


def test_a_waiting_ticket_may_not_resume_into_a_state_it_did_not_come_from() -> None:
    subject = ticket(status=Status.IN_REVIEW)
    transition(subject, Status.AWAITING_INPUT)

    with pytest.raises(IllegalTransitionError):
        transition(subject, Status.MERGING)

    assert subject.status is Status.AWAITING_INPUT
    assert subject.resume_to is Status.IN_REVIEW
